                ...
                ifelse(header_level == "h3", "#136CB9",
                       ifelse(header_level == "h4", "#CA3E34",
                              ifelse(header_level == "tip", "#2C5577",
                                     ifelse(header_level == "warning", "#775418", 
                                            ifelse(header_level == "white", "#FFFFFF",
                                                   ifelse(header_level == "spotify",  "#1DB954", "#2F4F4F"))))))))
}
...
get_tidy_text <- function(tidyobject, effect, digits = 2, p_digits = 3){
  row <- tidyobject %>% 
    dplyr::filter(term == effect)
  
  digits <- paste0("%.", digits, "f")
  p_digits <- paste0("%.", p_digits, "f")
  
  result <- paste0("*b* = ", sprintf(digits, row[, "estimate"]), " [", sprintf(digits, row[, "conf.low"]),  ", ", sprintf(digits, row[, "conf.high"]), "], *t* = ", sprintf(digits, row[, "statistic"]), ", *p* = ")
  if(row[, "p.value"] < 0.001){
    paste0(result, " < 0.001")
  } else {
    paste0(result, sprintf(p_digits, row[, "p.value"]))
  }
}

get_emm_text <- function(emmobject, var, gp, digits = 2){
  var = enquo(var)
  row <- emmobject %>% 
    dplyr::filter(!!var == gp)
  ...
  paste0("*M* = ", sprintf(digits, row["Mean"]), " [", sprintf(digits, row["CI_low"]), ", ", sprintf(digits, row["CI_high"]), "]")
}

get_lmrob_text <- function(lmrob_summary, effect, digits = 2, p_digits = 3){
  row <- lmrob_summary$coefficients %>%
    tibble::as_tibble() %>% 
  ...
  result <- paste0("*b* = ", sprintf(digits, row[, "estimate"]), ", *t* = ", sprintf(digits, row[, "statistic"]), ", *p* = ")
  if(row[, "p.value"] < 0.001){
    paste0(result, " < 0.001")
  } else {
    paste0(result, sprintf(p_digits, row[, "p.value"]))
...
get_param_text <- function(para_object, var, digits = 2, p_digits = 3){
  row <- para_object %>% 
    dplyr::filter(Parameter == var)
  
  digits <- paste0("%.", digits, "f")
  p_digits <- paste0("%.", p_digits, "f")
  
  result <- paste0("*b* = ", sprintf(digits, row[, "Coefficient"]), " [", sprintf(digits, row[, "CI_low"]),  ", ", sprintf(digits, row[, "CI_high"]), "], *t* = ", sprintf(digits, row[, "t"]), ", *p* = ")
  if(row[, "p"] < 0.001){
    paste0(result, " < 0.001")
  } else {
    paste0(result, sprintf(p_digits, row[, "p"]))
  }
  
}


# Create javascript to style font and background colour of data table headers

style_tbl_head <- function(head_txt = "#444444", head_bg = "#ffffff"){
  callback <- c(
    paste0("$('table.dataTable.display thead').css('background-color', '", head_bg, "')"),
    paste0("$('table.dataTable.display thead').css('color', '", head_txt, "')")
  )
  
  DT::JS(callback)
}  

# Create javascript to style font and background colour of data table rows

style_tbl_rows <- function(odd_bg = "#ffffff", even_bg = h1_col, odd_txt = "#444444", even_txt = "#444444", opacity = 20){
  
  even_bg <- paste0(even_bg, opacity)
  
  callback <- c(
    "function(row, data, num, index){",
    "  var $row = $(row);",
    "  if($row.hasClass('even')){",
    paste0("$row.css('background-color', '", even_bg, "');"),
    paste0("$row.css('color', '", even_txt, "')"),
    "  }else{",
    paste0("$row.css('background-color', '", odd_bg, "');"),
    paste0("$row.css('color', '", odd_txt, "')"),
    "  }",
    "}")
